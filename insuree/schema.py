...
class Query(ExportableQueryMixin, graphene.ObjectType):
    ...
    insuree_status_reasons = DjangoFilterConnectionField(
        InsureeStatusReasonGQLType, str=graphene.String()
    )
    ...
    def resolve_insurees(self, info, **kwargs):
        ...
        if not info.context.user._u.is_imis_admin and (
    ...
    def resolve_families(self, info, **kwargs):
        ...
        if not info.context.user._u.is_imis_admin:
            filters += [
                LocationManager().build_user_location_filter_query(
                    info.context.user._u,
                    prefix="location__parent__parent",
                    loc_types=["D"],
                )
            ]
    ...
    def resolve_insuree_policy(self, info, **kwargs):
        ...
        if parent_location is not None:
            ...
            filters += [
                (
                    Q(insuree__current_village__isnull=False)
                    & Q(**{current_village: parent_location})
                )
                | (
                    Q(insuree__current_village__isnull=True)
                    & Q(**{family_location: parent_location})
                )
            ]
...
def _insuree_additional_filters(sender, additional_filter, user):
    return _get_additional_filter(
        sender or Insuree, additional_filter, user, signal_before_insuree_policy_query
    )


def _insuree_insuree_additional_filters(sender, additional_filter, user):
    return _get_additional_filter(
        sender or InsureePolicy,
        additional_filter,
        user,
        signal_before_insuree_search_query,
    )
...
def _family_additional_filters(sender, additional_filter, user):
    return _get_additional_filter(
        sender or Family, additional_filter, user, signal_before_family_query
    )
