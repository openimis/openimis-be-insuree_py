...
def create_test_insuree(
    ...
    if insuree is None:
        ...
        if family_custom_props and (
            "location" in family_custom_props or "location_id" in family_custom_props
        ):
            family_location = (
                family_custom_props["location"]
                if "location" in family_custom_props
                else Location.objects.get(pk=family_custom_props["location_id"])
            )
        ...
        if custom_props and "current_village" in custom_props:
            ...
            village = qs_location.filter(
                current_village_id=custom_props.pop("current_village_id")
            ).first()
        ...
        insuree = Insuree.objects.create(
            ...
            other_names=get_from_custom_props(
                custom_props, "other_names", "First Second"
            ),
            family=family,
            gender=get_from_custom_props(
                custom_props, "gender", Gender.objects.get(code="M")
            ),
            dob=get_from_custom_props(custom_props, "dob", "1972-08-09"),
            chf_id=ref,
            head=is_head,
            card_issued=get_from_custom_props(custom_props, "card_issued", True),
            validity_from=get_from_custom_props(
                custom_props, "validity_from", "2019-01-01"
            ),
    ...
    if with_family and family is None and insuree.family is None:
        if not family_custom_props:
            family_custom_props = {}
        if (
            "head_insuree" not in family_custom_props
            and "head_insuree_id" not in family_custom_props
        ):
    ...
    if family is None:
        ...
        if custom_props and "location" in custom_props:
            ...
            location = qs_location.filter(
                location_id=custom_props.pop("location_id")
            ).first()
        else:
